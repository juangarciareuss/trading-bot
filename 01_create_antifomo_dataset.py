...
def fetch_data_from_local_files(symbol):
    ...
    # Aseguramos tener todos los timeframes necesarios para las features
    timeframes = ['1w', '1d', '4h', '1h', '5m']
    data = {}
    print(f"  Leyendo datos locales para {symbol}...")
    
    base_path = "Backtest/data/historico"
    # Formatear el símbolo para que coincida con el nombre del archivo (ej. 'BTC/USDT' -> 'BTCUSDT')
    formatted_symbol = symbol.replace('/', '')

    for tf in timeframes:
        # Construimos la ruta al archivo, ej: Backtest/data/historico/1h/BTCUSDT.csv
        file_path = os.path.join(base_path, tf, f"{formatted_symbol}.csv")
        try:
            df = pd.read_csv(file_path)
            # Asumimos que la primera columna es el timestamp y le damos el nombre correcto
            df.rename(columns={df.columns[0]: 'timestamp'}, inplace=True)
            df['timestamp'] = pd.to_datetime(df['timestamp'])
            df.set_index('timestamp', inplace=True)
            # Renombramos las columnas por si tienen nombres distintos (ej. 'Open', 'close')
...
def create_advanced_features(altcoin_data, btc_data, eth_data):
    ...
    # Feature de Máximo Histórico (ATH)
    df['ath'] = df['high'].cummax()

    # Features de Momentum del Activo
    ...
    # Features de Contexto (RSI en 1W, 1D, 4H)
...
if __name__ == "__main__":
    ...
    all_datasets = []
    # Creamos una copia de la lista de SÍMBOLOS para poder iterar sobre ella
    symbols_to_process = SYMBOLS[:]
    # Excluimos BTC y ETH del procesamiento principal de altcoins
    ...
    if not all_datasets:
        ...
        # Eliminar filas con valores nulos que no se pudieron rellenar
